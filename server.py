...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/flight-icon-180.png":
            return self._file(os.path.join(HERE, "flight-icon-180.png"), "image/png")
        if p == "/flight-sw.js":
            return self._file(os.path.join(HERE, "flight-sw.js"), "application/javascript; charset=utf-8")
        if p == "/flight-manifest.json":
            return self._file(os.path.join(HERE, "flight-manifest.json"), "application/manifest+json; charset=utf-8")
    ...
    @property
    def STUDENT(self):
        """시험 전/후로 학생 상황이 완전히 달라진다. 날짜로 판단해 맥락을 바꾼다."""
        if datetime.date.today() <= datetime.date(2026, 8, 11):
            return ("학생 이름은 '수영'. 2026-08-11(화) 고졸 검정고시 재응시. 영어는 면제(100점 확정), "
                    "응시 6과목은 국어·수학·사회·과학·한국사·도덕. 취약 과목은 과학·도덕·수학. "
                    "목표는 평균 94점 이상(성공회대 열린인재 지원선). 반말로 친근하게 대한다.")
        return ("학생 이름은 '수영'. 2026-08-11 고졸 검정고시를 이미 봤고 시험은 끝났다. "
                "지금은 성공회대 2027 수시(열린인재전형)를 준비하는 중이고, 다음 할 일은 "
                "학교생활기록부 대체서식 작성과 10월 31일 면접 준비다. "
                "시험 과목 공부를 하라고 하지 마라. 한국사만 본인이 다시 보고 싶어 한다. "
                "반말로 친근하게 대한다.")
    ...
    def _focus_check(self, b):
        ...
        prompt = (f"{self.STUDENT}\n{what}\n"
                  ...
                  "message에 구체적인 과목명이나 할 일을 지어내지 마세요. 화면에서 실제로 보이는 것만 근거로 쓰세요.")
        try:
